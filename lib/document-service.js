...
function buildStarterProject() {
  ...
  return {
    lineEnding: '\n',
    title: 'New Project',
    milestoneColumnWidths: [6, 19, 12, 13],
    milestones: [
      {
        id: 'M1',
    ...
    todos: {
      description: 'Quick tasks and reminders not tied to a kanban issue.',
      items: [
        {
          text: 'Example todo',
          done: false,
...
export class ProjectDocumentService extends EventEmitter {
  ...
  async createProject(filePath) {
    if (!filePath) {
      throw new Error('No project file selected');
    }

    const serialized = serialize(buildStarterProject());
    this.#writeFileAt(filePath, serialized);

    return this.openProject(filePath);
  }
  ...
  #writeRaw(content) {
    this.#writeFileAt(this.#projectFile, content, { markInternalChange: true });
  }

  #writeFileAt(filePath, content, { markInternalChange = false } = {}) {
    const tmp = `${filePath}.tmp`;
    if (markInternalChange) {
